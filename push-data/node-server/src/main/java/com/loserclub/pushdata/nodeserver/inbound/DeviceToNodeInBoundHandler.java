...
@Component
public class DeviceToNodeInBoundHandler extends SimpleChannelInboundHandler<String> {
    @Autowired
    private List<IDeviceDataHandler> deviceDataHandlers;

    @Autowired
    private InActiveHandlerForDevice inActiveHandler;

    @Override
    protected void channelRead0(ChannelHandlerContext channelHandlerContext, String message) throws Exception {
        NodeMessage nodeMessage = NodeMessage.decode(message);

        if (!deviceDataHandlers.isEmpty()) {
            deviceDataHandlers.stream().forEach((handler) -> {
                try {
